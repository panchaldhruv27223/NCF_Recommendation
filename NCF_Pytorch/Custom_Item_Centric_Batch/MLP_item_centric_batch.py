...
def main(learner = 'adam', layers= [32, 16, 8],epochs = 3, batch_size= 256, num_factors = 10, num_neg = 2, topK= 10, shuffle=False, shuffle_items=True, shuffle_within_item=True, output_folder_path="", output_folder_path_log = ""):
    ...
    train_logger.info("Starting MLP Item Centric traning... ")

    eval_logger, eval_logger_path = setup_logger(output_folder_path_log, "evaluation", config=configurations)

    eval_logger.info("Starting MLP Item Centric Evaluation")
    ...
    train_logger.info("MLP Item Centric Model loaded.")
    eval_logger.info("MLP Item Centric Model loaded.")
    ...
    train_logger.info(f"MLP Item Centric Model passed for training...")
    eval_logger.info("MLP Item Centric Model Passed for Evaluation...")
...
if __name__ == "__main__":
    print("Calling from MLP Item Centric Training.")
    
    main(learner = 'adam', layers= [32, 16, 8], epochs = 10, batch_size= -1, num_factors = 10, num_neg = -1, topK= 10, shuffle=False, shuffle_items=True, shuffle_within_item=True, output_folder_path="MLP_Item_centric_pos_neg_eq_per_item", output_folder_path_log = "MLP_Item_centric_pos_neg_eq_per_item")